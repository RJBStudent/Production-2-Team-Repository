...
public class BourdelaisCDEPlayerMovementScript : MonoBehaviour
{
   ...
   void GroundCheck()
   {
      ...
      for (int i = 0; i < hitCollide.Length && !inAir; i++)
      {
         ...
         {
                if (transform.parent != hitCollide[i].gameObject.transform)
                {
   ...
   void LoadLevel()
   {
    ...
    void ResetScale()
    {
        ...
        transform.localScale = new Vector3(1/transform.parent.lossyScale.x, 1/transform.parent.lossyScale.y, 1/transform.parent.lossyScale.y);
    }
}
